...
- docker run -d --name Pokemap --link mysql:mysql --expose=80 -v config_stachus.ini:/PokemonGo-Map/config/config.ini scazadar/pokemap -se -L de

Starts second server without webserver:
- docker run -d --name Pokemap --link mysql:mysql --expose=80 -v config_marienplatz.ini:/PokemonGo-Map/config/config.ini scazadar/pokemap -se -L de -ns
